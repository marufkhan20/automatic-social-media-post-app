...
const EditPost = ({ handleCancel, refetch, data }) => {
  // create post state
  ...
  useEffect(() => {
    if (data?._id) {
      setDate(data?.date);
      setExactTime(data?.time);
      setTitle(data?.title);
      setSelectedOption(data?.attachmentType);

      data?.attachmentType === "video"
        ? setVideos(data?.attachments)
        : data?.attachmentType === "image"
        ? setImages(data?.attachments)
        : data?.attachmentType === "album"
        ? setAlbum(data?.attachments[0])
        : data?.attachmentType === "link"
        ? setWebLink(data?.attachments[0])
        : setAlbum("");
    }
  }, [data]);
  ...
  // create new post
  const [updatePost, { data: updatedPost, isLoading, isError, error }] =
  ...
  useEffect(() => {
    ...
    if (!isLoading && !isError && updatedPost?._id) {
      toast.success("Post Updated Successfully");
      handleCancel();
      setErrors({});
      refetch();
    }
  }, [updatedPost, isLoading, isError, error, handleCancel, refetch]);
  ...
  return (
    <div>
      <div className="bg-[#012B6D] Z-[999] w-[328px] md:w-[700px] fixed rounded-t-md p-5 ml-[-1.5rem] mt-[-1.3rem]">
        <h1 className="text-[25px] text-white">Edit Post</h1>
      </div>
