...
const Join = () => {
  ...
  useEffect(() => {
    if (!joinTeamLoading && joinTeamIsError) {
      const { data } = joinTeamError || {};
      setTeamError(data.error?.code);
    }

    if (!joinTeamLoading && !joinTeamIsError && joinedTeam?._id) {
      toast.success(
        "Team Join Request Sent Successfully. Wait For Admin Approval."
      );
      setTeamCode("");
      setTeamError("");
    }
  }, [joinedTeam, joinTeamLoading, joinTeamIsError, joinTeamError]);

  // submit handler
  const joinTeamSubmitHandler = (e) => {
    e.preventDefault();

    // check validation
    if (!teamCode) {
      return setTeamError("Team Code is required!!");
    }
  ...
  // create new team
  const [createTeam, { data: team, isLoading, isError, error }] =
    useCreateTeamMutation();
        ...
        <div className="flex flex-col gap-y-10 items-start justify-start w-full">
          ...
          >
            ...
            <p className="text-[18px]">
              A team allows its members to share templates and images without
              needing a share code. If there's a team you want to join, enter
              their code below
            </p>
          ...
          >
            ...
            <div className="text-[18px] pb-5 flex flex-col gap-3">
              <p className="pt-5">
                Create a team so you can share templates and images with your
                teammates.
              </p>
              <div className="mt-5">
                <button
                  onClick={showModal}
                  className="bg-[#FF5FC0] rounded-md w-[188px] h-[35px] text-white font-[500]"
                >
                  Create a New Team
                </button>
              </div>
