...
import { useGetSingleTeamQuery } from "../../../features/team/teamApi";
...
const Videos = () => {
  ...
  // get team
  const { data: team } = useGetSingleTeamQuery(id);

  // get all video galleries
  const { data: galleries, isLoading: loading } = useGetGalleriesQuery({
    type: "video",
    team: id,
  });
      ...
      <DashboardLayout>
              ...
              <div className="grid grid-cols-6 gap-5">
                {!loading &&
                  ...
                  galleries
                    ...
                    .map((gallery) => {
                      return gallery?.resources?.map((video) => (
                        <div
                          key={video}
        ...
        >
          ...
          <div className="my-10 text-[#4A4A4A] flex flex-col justify-center items-center w-full">
            <video
              src={`${process.env.REACT_APP_API_URL}${viewVideo?.video}`}
              controls
            ></video>
          </div>
        </Modal>
