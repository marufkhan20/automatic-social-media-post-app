...
const Upload2 = ({ galleries, handleCancel }) => {
  ...
  useEffect(() => {
    ...
    if (!isLoading && !isError && gallery?._id) {
      toast.success("Gallery Image Upload Successfully");
      setGalleryId("");
      setGalleryImage("");
      setErrors({});
      handleCancel();
  ...
  const submitHandler = (e) => {
    ...
    if (!galleryImage) {
      validationErrors.galleryImage = "Image is required!!";
    }
  ...
  return (
    <div className="flex flex-col justify-center gap-5 items-center">
      ...
      {errors?.galleryImage && (
        <p className="text-red-500 font-medium">{errors?.galleryImage}</p>
      )}

      {galleryImage && (
        <div>
          <img className="mx-auto h-[200px]" src={galleryImage} alt="" />
        </div>
      )}
