...
const AdminHome = () => {
  ...
  // Convert the dates to a string for display or further processing
  const formattedCurrentDate = currentDate.toISOString().split("T")[0];
  const formattedThirtyDaysAgo = thirtyDaysAgo.toISOString().split("T")[0];

  console.log("Current Date:", formattedCurrentDate);
  console.log("Date 30 Days Ago:", formattedThirtyDaysAgo);
  return (
    <div className=" flex flex-col gap-5 w-full p-4">
      <div className="flex md:items-center md:justify-between w-full text-white bg-[#FF5FC0]">
        ...
        <div>
          <img
            ...
            alt=""
            className="w-[290px] md:block hidden"
          />
        </div>
      </div>
      <div className="mt-6">
        <div className="flex md:flex-row flex-col md:items-center gap-5">
          <div className="px-5 py-3 border rounded-md text-white border-[#B2B2B2]  bg-[#20293A]">
            {date.format(new Date(thirtyDaysAgo), "MMM DD, YYYY")} -{" "}
            {date.format(new Date(currentDate), "MMM DD, YYYY")}
          </div>
          <Link to={"/add-user"}>
      ...
      <div className="mt-6 flex items-center">
        <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-5 w-full">
          <div
            className="p-5 border rounded-lg w-full md:w-[380px] border-[#B2B2B2] bg-[#20293A]"
            style={{ boxShadow: "0px 4px 4px 0px rgba(0, 0, 0, 0.25)" }}
          >
            <div className="flex flex-col gap-2 text-white">
              <p className="text-[20px] font-[700]">Social Reach Pro</p>
              <p className="text-[20px] font-[400]">Sales</p>
          ...
          >
            <div className="flex flex-col gap-2 text-white">
              <p className="text-[20px] font-[700]">Social Reach Pro</p>
              <p className="text-[20px] font-[400]">Users</p>
              <div className="flex items-center gap-3">
                <p className="text-[36px] font-[800]">
                  {information?.users?.length} Users
                </p>
              </div>
          ...
          >
            <div className="flex flex-col gap-2 text-white">
              <p className="text-[20px] font-[700]">Social Reach Pro</p>
              <p className="text-[20px] font-[400]">Managers</p>
              <div className="flex items-center gap-3">
                <p className="text-[36px] font-[800]">
                  {information?.managers?.length} Managers
                </p>
              </div>
      ...
      <div className="flex md:flex-row flex-col mt-5 gap-3 text-white w-full">
        <div className="w-full  md:w-1/2">
          <div className="p-3 overflow-x-scroll overflow-y-hidden md:overflow-hidden  border rounded-lg border-[#B2B2B2] bg-[#20293A]">
            <p className="text-[20px] font-[600] pb-5">Customers</p>
            <div className="border-b w-full border  border-[#B2B2B2]/50 "></div>
            <div className="pt-3">
              <CustomerTable data={information?.users} />
            </div>
          </div>
        </div>
        <div className="w-full overflow-x-scroll overflow-y-hidden md:overflow-hidden md:w-1/2 p-3 border rounded-lg  border-[#B2B2B2] bg-[#20293A]">
          <p className="text-[20px] font-[600] pb-5">Managers</p>
          <div className="border-b w-full border  border-[#B2B2B2]/50 "></div>
          <div className="pt-3">
            <CustomerTable data={information?.managers} />
          </div>
        </div>
      </div>
    </div>
